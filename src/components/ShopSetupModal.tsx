...
export default function ShopSetupModal({ uid, profile, onComplete }: ShopSetupModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-stone-900/60 backdrop-blur-md animate-in fade-in duration-500">
      <div className="bg-white w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 slide-in-from-bottom-10 duration-500">
        ...
        <div className="p-8 text-center">
          <div className="flex justify-center mb-4">
            <span className="flex items-center gap-2 px-3 py-1 bg-emerald-50 text-emerald-700 text-xs font-bold rounded-full uppercase tracking-widest">
              <Sparkles className="w-3 h-3" />
              Getting Started
            </span>
          </div>
          
          <h2 className="text-2xl font-bold text-stone-900 mb-2">Welcome, {profile.name}!</h2>
          <p className="text-stone-500 mb-8">Let's set up your shop profile to get started with Store Mate.</p>
          
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="text-left space-y-2">
              <label className="text-sm font-bold text-stone-700 ml-1">What's your shop's name?</label>
              <input
                type="text"
                value={shopName}
                onChange={(e) => setShopName(e.target.value)}
                placeholder="e.g. City Grocery Store"
                className="w-full px-5 py-4 bg-stone-50 border border-stone-200 rounded-2xl focus:ring-2 focus:ring-emerald-500 focus:border-transparent outline-none transition-all text-lg font-medium"
                autoFocus
              />
            </div>
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-3 bg-emerald-600 text-white py-4 px-6 rounded-2xl font-bold hover:bg-emerald-700 transition-all shadow-xl shadow-emerald-100 active:scale-95 disabled:opacity-50 disabled:active:scale-100"
            >
